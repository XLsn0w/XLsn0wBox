...
- (instancetype)initWithFrame:(CGRect)frame norImageArr:(NSArray *)norImageArr SelImageArr:(NSArray *)selImageArr TitleArr:(NSArray *)titleArr Config:(AniTabBarShared *)config{
    if (self = [super initWithFrame:frame]) {        
        for (int i = 0; i < titleArr.count; i++) {
            ...
            tbBtn.title.textColor = [AniShared norTitleColor];
        ...
        //背景颜色处理
        self.backgroundColor = [AniShared tabBarBackground];
...
- (void)tapClick:(UITapGestureRecognizer *)tap {
    ...
    if ([self.ani_delegate respondsToSelector:@selector(tabBar:didSelectIndex:)]) {
        [self.ani_delegate tabBar:self didSelectIndex:tap.view.tag];
    }
}
...
- (void)setUpSelectedIndex:(NSInteger)selectedIndex {
    for (int i = 0; i < self.saveTabBarArrM.count; i++) {
        ...
        if (i == selectedIndex) {
            tbBtn.title.textColor = [AniShared selTitleColor];
            tbBtn.imageView.image = [UIImage imageNamed:self.selImageArrM[i]];
            
            JMConfigTabBarAnimType type = [AniShared tabBarAnimType];
...
- (void)topLineIsClearColor:(BOOL)isClearColor {
    ...
    if (!isClearColor) {
        color = [AniShared tabBarTopLineColor];
    }
