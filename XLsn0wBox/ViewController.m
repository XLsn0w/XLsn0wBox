...
/** tabbar */
@property (nonatomic, strong) AniTabBarController *tabBarVc;
/** 强引用 */
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        if (indexPath.row == 0) { //默认样式(不做任何配置)
            ...
            AniShared.typeLayout = JMConfigTypeLayoutImage;
        } else if (indexPath.row == 2) {
            AniShared.typeLayout = JMConfigTypeLayoutImage;
            AniShared.imageSize = CGSizeMake(38, 38);
        } else if (indexPath.row == 3) {
            AniShared.tabBarAnimType = JMConfigTabBarAnimTypeRotationY;
        } else if (indexPath.row == 4) {
            AniShared.tabBarAnimType = JMConfigTabBarAnimTypeScale;
        } else if (indexPath.row == 5) {
            AniShared.tabBarAnimType = JMConfigTabBarAnimTypeBoundsMax;
        } else if (indexPath.row == 6) {
            AniShared.tabBarAnimType = JMConfigTabBarAnimTypeBoundsMin;
        } else if (indexPath.row == 7) {
            AniShared.norTitleColor = [UIColor blueColor];
            AniShared.selTitleColor = [UIColor redColor];
        } else if (indexPath.row == 8) {
            AniShared.tabBarBackground = [UIColor greenColor];
        }
    } else if (indexPath.section == 1) {
        //关于badgeValue的样式可全局配置 ,所以这里就return了
        if (indexPath.row == 0) {
//            [config showNewBadgeAtIndex:0];
            [AniShared showNewBadgeAtIndex:1];
        } else if (indexPath.row == 1) {
            [AniShared showPointBadgeAtIndex:2];
        } else if (indexPath.row == 2) {
            [AniShared showNumberBadgeValue:@"6" AtIndex:3];
            [AniShared showNumberBadgeValue:@"99" AtIndex:4];
        } else if (indexPath.row == 3) {
            AniShared.animType = JMConfigBadgeAnimTypeShake;
            [AniShared showNumberBadgeValue:@"99" AtIndex:4];
        } else if (indexPath.row == 4) {
            AniShared.animType = JMConfigBadgeAnimTypeOpacity;
            [AniShared showNumberBadgeValue:@"99" AtIndex:4];
        } else if (indexPath.row == 5) {
            AniShared.animType = JMConfigBadgeAnimTypeScale;
            [AniShared showNumberBadgeValue:@"99" AtIndex:4];
        }
    ...
    for (int i = 0; i < titleArr.count; i++) {
     ...
     self.tabBarVc = [[AniTabBarController alloc] initWithTabBarControllers:controllersArr
                                                               NorImageArr:imageNormalArr
                                                               SelImageArr:imageSelectedArr
                                                                  TitleArr:titleArr
                                                                    Config:AniShared];
